            ...
            images.map((image, index) => {
              return(
                <div class={`carousel-item ${index === 0 && "active"} ${el.brand === "Dolce & Gabbana" && "dolce"} ${el.brand === "Louis Vuitton" && "lv"} ${ourStore && "ourStore"}`}>
                  {
                    image.includes("mp4") ? (
                      // <div className="colSliderLoader">
                      //   {
                      //   }
                      // </div>
                      <video src={image.trim()} muted controls loop className='d-block w-100' onClick={()=> window.location.assign(`https://madeinheaven-mgf.netlify.app/product/${el.id}`)} style={{cursor:"pointer"}}></video>
                    ):(
                      // <div className='colSliderLoader'>
                      //   {
                      //   }
                      // </div>
                      image.slice(0, 2) === "hh" ? <img src={"h" + image.slice(2, image.length).trim()} class="d-block w-100" alt="..."  onClick={()=> window.location.assign(`https://madeinheaven-mgf.netlify.app/product/${el.id}`)}/> : <img src={image.trim()} class="d-block w-100" alt="..."  onClick={()=> window.location.assign(`https://madeinheaven-mgf.netlify.app/product/${el.id}`)}/>
                    )
                  }
                </div>
              )
