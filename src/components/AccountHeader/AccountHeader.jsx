...
const AccountHeader = ({refHeader, dark, noBorder, absolute, light, moreTop, moreTopWhite}) => {
  ...
  return ( 
    <div className={`${styles.wrapper} ${dark && styles.dark} ${light && styles.whiteHeader} ${absolute && styles.absolute}`} ref={refHeader}>
      
      <LoginModal/>
      <CartModal/>
      <CollectionsModal/>
      <LanguageModal moreTop={moreTop ? true : false} moreTopWhite={moreTopWhite ? true : false}/>
      <SearchModal/>
      <SearchBoxModalLeft/>
      <SearchBoxModalRight/>
      ...
      <div className={`${styles.header}`} style={{borderBottom: noBorder ? "#19110b" : "#eae8e4"}}>
        <div className={styles.left}>
          ...
          <span className={styles.linkSpan} onClick={()=> navigate("/home")} style={{opacity: showLanguageModal ? "0" : "1"}}><Translate>COLLECTIONS</Translate></span>
          <span className={`${styles.linkSpan} ${styles.searchSpan}`} style={{opacity: showLanguageModal ? "0" : "1"}} onClick={()=> setShowSearchModal(!showSearchModal)} data-search><Translate>SEARCH</Translate></span>
        </div>
